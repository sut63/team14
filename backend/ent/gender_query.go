...
func (gq *GenderQuery) QueryPersonal() *PersonalQuery {
	query := &PersonalQuery{config: gq.config}
...
func (gq *GenderQuery) WithPersonal(opts ...func(*PersonalQuery)) *GenderQuery {
	query := &PersonalQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withPersonal = query
	return gq
}
...
func (gq *GenderQuery) sqlAll(ctx context.Context) ([]*Gender, error) {
	...
	if query := gq.withPersonal; query != nil {
		...
		query.Where(predicate.Personal(func(s *sql.Selector) {
			s.Where(sql.InValues(gender.PersonalColumn, fks...))
		}))
