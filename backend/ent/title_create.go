...
func (tc *TitleCreate) AddPersonalIDs(ids ...int) *TitleCreate {
	tc.mutation.AddPersonalIDs(ids...)
	return tc
}

// AddPersonal adds the personal edges to Personal.
func (tc *TitleCreate) AddPersonal(p ...*Personal) *TitleCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (tc *TitleCreate) createSpec() (*Title, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.PersonalIDs(); len(nodes) > 0 {
