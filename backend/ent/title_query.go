...
func (tq *TitleQuery) QueryPersonal() *PersonalQuery {
	query := &PersonalQuery{config: tq.config}
...
func (tq *TitleQuery) WithPersonal(opts ...func(*PersonalQuery)) *TitleQuery {
	query := &PersonalQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withPersonal = query
	return tq
}
...
func (tq *TitleQuery) sqlAll(ctx context.Context) ([]*Title, error) {
	...
	if query := tq.withPersonal; query != nil {
		...
		query.Where(predicate.Personal(func(s *sql.Selector) {
			s.Where(sql.InValues(title.PersonalColumn, fks...))
		}))
