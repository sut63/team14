...
func (dq *DepartmentQuery) QueryPersonal() *PersonalQuery {
	query := &PersonalQuery{config: dq.config}
...
func (dq *DepartmentQuery) QueryCustomer() *CustomerQuery {
	query := &CustomerQuery{config: dq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DepartmentQuery) WithPersonal(opts ...func(*PersonalQuery)) *DepartmentQuery {
	query := &PersonalQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withPersonal = query
	return dq
}
...
func (dq *DepartmentQuery) WithCustomer(opts ...func(*CustomerQuery)) *DepartmentQuery {
	query := &CustomerQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context) ([]*Department, error) {
	...
	if query := dq.withPersonal; query != nil {
		...
		query.withFKs = true
		query.Where(predicate.Personal(func(s *sql.Selector) {
			s.Where(sql.InValues(department.PersonalColumn, fks...))
		}))
	...
	if query := dq.withCustomer; query != nil {
		...
		query.Where(predicate.Customer(func(s *sql.Selector) {
			s.Where(sql.InValues(department.CustomerColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
